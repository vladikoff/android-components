 ...
 */
class MinimalLoginsExample(private val databasePath: String) {
    ...
    private fun getUnlockedStore(): SyncResult<LoginsStorage> {
        return if (this.unlockedStore != null) {
            ...
            val store = MentatLoginsStorage(this.databasePath)
    ...
    private fun getUnlockInfo(oauthInfo: OAuthInfo, tokenServerURL: String): SyncUnlockInfo {
        // There's probably a less convoluted way to do this...
        val stringBuilder: StringBuilder = StringBuilder(oauthInfo.keys!!)
        val keysObj = Parser().parse(stringBuilder) as JsonObject
        val keyInfo = keysObj.obj("https://identity.mozilla.com/apps/oldsync")!!
        return SyncUnlockInfo(
                kid            = keyInfo.string("kid")!!,
                fxaAccessToken = oauthInfo.accessToken!!,
                syncKey        = keyInfo.string("k")!!,
