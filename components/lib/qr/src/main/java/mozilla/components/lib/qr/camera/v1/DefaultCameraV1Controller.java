package mozilla.components.lib.qr.camera.v1;
...
import mozilla.components.lib.qr.camera.AbstractController;
import mozilla.components.lib.qr.camera.CAMViewAsyncTask;
...
public class DefaultCameraV1Controller extends AbstractController implements Camera.PreviewCallback
{
    ...
    public void startPreview(SurfaceView surfaceView) throws IOException
    {
        if (isCameraReadyForUserOperations())
        {
            ...
            catch (Throwable err)
            {
                ...
                try
                {
                    Camera.Parameters parameters = CameraUtilsV1.getFailsafeCameraParameters(rawCameraObject);
                    parameters.setPreviewFormat(previewFormat);
                    rawCameraObject.setParameters(parameters);
                }
    ...
    private void rechargePreviewBuffer()
    {
        ...
        final Camera.Size size = rawCameraObject.getParameters().getPreviewSize();
    ...
    public void switchFlashlight(boolean turnOn)
    {
        if (isCameraReadyForUserOperations())
        {
            final Camera.Parameters parameters = rawCameraObject.getParameters();
            ...
            new CAMViewAsyncTask()
            {
                ...
                protected Object doInBackground(Object[] args) throws Throwable
                {
                    ...
                    while (rawCameraObject == null && retriesCount > 0)
                    {
                        try
                        {
                            rawCameraObject = Camera.open(Integer.parseInt(camera.getCameraId()));
                        }
